...
-- Yeni unique constraint ekle: aynı oturum ve öğrenci için tek kayıt
CREATE UNIQUE INDEX "yoklamalar_oturum_id_ogrenci_id_unique" ON "yoklamalar"("oturum_id", "ogrenci_id") WHERE "tur_no" IS NULL;
...
-- GÜNCELLEME: ogrenci_id sütunu ekle
ALTER TABLE "ders_kayitlari" ADD COLUMN IF NOT EXISTS "ogrenci_id" INTEGER;

-- Eski unique constraint ve foreign keyleri kaldır
DROP INDEX IF EXISTS "ders_kayitlari_ders_id_universite_kodu_key";
ALTER TABLE "ders_kayitlari" DROP CONSTRAINT IF EXISTS "ders_kayitlari_universite_kodu_fkey";

-- ogrenci_id için foreign key ekle
ALTER TABLE "ders_kayitlari" ADD CONSTRAINT IF NOT EXISTS "ders_kayitlari_ogrenci_id_fkey" FOREIGN KEY ("ogrenci_id") REFERENCES "kullanicilar"("id") ON DELETE CASCADE ON UPDATE NO ACTION;

-- Yeni unique constraint
CREATE UNIQUE INDEX IF NOT EXISTS "ders_kayitlari_ders_id_ogrenci_id_key" ON "ders_kayitlari"("ders_id", "ogrenci_id");

-- Eski kayıtlardaki ogrenci_id değerlerini doldur (universite_kodu ile eşleştir)
UPDATE "ders_kayitlari" dk
SET "ogrenci_id" = k."id"
FROM "kullanicilar" k
WHERE dk."universite_kodu" = k."universite_kodu";

-- Unique constraint güncelle
DROP INDEX IF EXISTS "ders_kayitlari_ders_id_ogrenci_id_key";
DROP INDEX IF EXISTS "ders_kayitlari_ders_id_universite_kodu_key";
CREATE UNIQUE INDEX IF NOT EXISTS "ders_kayitlari_ders_id_ogrenci_id_alinma_tipi_key" ON "ders_kayitlari"("ders_id", "ogrenci_id", "alinma_tipi");
