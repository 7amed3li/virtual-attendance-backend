...
const DepartmentAddPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchFaculties = async () => {
      try {
        ...
        setFaculties(data.map((f: any) => ({ label: f.ad, value: f.id })));
      } catch (err) {
        console.error('Error fetching faculties:', err);
        message.error(t('error_loading_faculties'));
      } finally {
  ...
  const onFinish = async (values: { ad: string; fakulte_id: number }) => {
    ...
    try {
      ...
      message.success(t('department_added_successfully'));
      navigate('/departments');
    } catch (err) {
      console.error('Error adding department:', err);
      message.error(t('error_adding_department'));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
          <div>
            ...
            <Text type="secondary">{t('add_new_department_to_faculty')}</Text>
          </div>
          <Button 
        ...
        <Breadcrumb>
          ...
          <Breadcrumb.Item>{t('add_department')}</Breadcrumb.Item>
        </Breadcrumb>
      </div>
      ...
      <Row gutter={[24, 24]}>
        ...
        <Col xs={24} lg={16}>
          <Card>
            <Alert
              message={t('department_creation_notice')}
              description={t('department_creation_notice_description')}
              type="info"
              showIcon
            ...
            >
              ...
              <Form.Item style={{ marginTop: '32px', marginBottom: 0 }}>
                <Space size="middle">
                  ...
                  >
                    {loading ? t('adding_department') : t('add_department')}
                  </Button>
                  <Button 
          ...
          >
            <div style={{ textAlign: 'center', marginBottom: '24px' }}>
              ...
              <div>
                <Text strong style={{ fontSize: '16px' }}>
                  {form.getFieldValue('ad') || t('department_name')}
                </Text>
                <br />
                <Text type="secondary">
            ...
            <Space direction="vertical" size="small" style={{ width: '100%' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <Text style={{ color: '#52c41a' }}>{t('will_be_active')}</Text>
              </div>
            </Space>
            ...
            <Alert
              message={t('department_will_be_active')}
              description={t('department_will_be_available_for_courses')}
              type="success"
              showIcon
            />
            ...
            {faculties.length === 0 && (
              <Alert
                message={t('no_faculties_found')}
                description={t('please_add_faculty_first')}
                type="warning"
                showIcon
